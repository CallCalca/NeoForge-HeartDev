...
# HeartDev
The API allows developers to easily access vanilla **health bar rendering systems**. It provides a wide range of tools to customize textures and behaviors, needing only a small setup.

## Understanding the library
Now that the event class has been successfully created, let's start using the tools this library provides.
First, let's divide the available tools according to their task:
- **Rendering methods** : all that is placed within the HealthBar class takes care of the rendering itself.
- **Manipulation methods** : these methods are stored in the HealthComponent class, and they are the ones that provide access to textures and behaviors.
- **Creation methods** : these methods can be found in the HealthTypes class and allow you to create new HeartTypes (containers with all the different states a heart can take, such as full, half, blinking, etc...).
